...
const ScanFridgePanel = ({ isScanning, onFileSelect }: Props) => {
  ...
  return (
    <div
      className={`fixed top-4 left-4 z-50 retro-panel w-56 transition-all duration-300 ${
        isScanning ? 'scale-[1.03] shadow-[0_0_24px_rgba(255,220,100,0.45)]' : ''
      }`}
    >
      <div className="retro-title mb-3">{isScanning ? 'SCANNING...' : 'SCAN FRIDGE'}</div>
